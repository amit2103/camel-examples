...
import org.springframework.jms.connection.ConnectionFactoryUtils.ResourceFactory;
...
public class TestDMLC extends DefaultMessageListenerContainer {

	/**
	 * Default thread name prefix: "DefaultMessageListenerContainer-".
	 */
	public static final String DEFAULT_THREAD_NAME_PREFIX =
			ClassUtils.getShortName(TestDMLC.class) + "-";
	 ...
	 private ThreadLocal<MessageConsumer> consumerThreadLocal =
	...
	private ResourceFactory transactionalResourceFactory = new MessageListenerContainerResourceFactory();
...
//			JmsUtils.closeSession(sessionToClose);
	...
	protected Session createSession(Connection con) throws JMSException {
		...
		if(session == null) {
		   session =  con.createSession(isSessionTransacted(), getSessionAcknowledgeMode());
	...
	protected MessageConsumer createListenerConsumer(Session session) throws JMSException {
		...
		MessageConsumer consumer = consumerThreadLocal.get();
		if(consumer == null) {
	...
	private class MessageListenerContainerResourceFactory implements ConnectionFactoryUtils.ResourceFactory {
		...
		public Connection getConnection(JmsResourceHolder holder) {
			return TestDMLC.this.getConnection(holder);
		}

		@Override
		public Session getSession(JmsResourceHolder holder) {
			return TestDMLC.this.getSession(holder);
		}

		@Override
		public Connection createConnection() throws JMSException {
			if (TestDMLC.this.sharedConnectionEnabled()) {
				Connection sharedCon = TestDMLC.this.getSharedConnection();
				return new SingleConnectionFactory(sharedCon).createConnection();
			}
			else {
				return TestDMLC.this.createConnection();
			}
		}

		@Override
		public Session createSession(Connection con) throws JMSException {
			return TestDMLC.this.createSession(con);
		}

		@Override
		public boolean isSynchedLocalTransactionAllowed() {
			return TestDMLC.this.isSessionTransacted();
		}
	}
